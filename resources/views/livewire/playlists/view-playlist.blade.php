<div class="bg-gradient-to-b from-gray-800 to-container-spotify relative">
            ...
            <div class="flex-shrink-0">
                @if(isset($this->playlistInfo['image']))
                    <img src="{{ $this->playlistInfo['image'] }}" alt="image-playlist"
                         class="w-60 h-60 rounded-lg shadow-2xl flex items-center justify-center object-cover">
                @else
                    <div
                        class="w-52 h-52 bg-gradient-to-br from-purple-500 to-pink-500 rounded-lg shadow-2xl flex items-center justify-center">
                ...
                <a href="{{ route('new-musics-playlist', ['id' => $playlistId]) }}" wire:navigate class="add-music-card rounded-xl p-6 cursor-pointer group">
                    <div class="flex items-center gap-4">
                        <div class="relative">
                            ...
                            <div
                                class="floating-icon w-16 h-16 bg-gradient-to-br from-purple-500 to-pink-500 rounded-full flex items-center justify-center shadow-lg">
                ...
                <div wire:click="openModalFavoriteMusics" class="liked-music-card rounded-xl p-6 cursor-pointer group">
                    <div class="flex items-center gap-4">
                        <div class="relative">
                            ...
                            <div
                                class="floating-icon w-16 h-16 bg-gradient-to-br from-green-500 to-blue-500 rounded-full flex items-center justify-center shadow-lg">
                        ...
                        <div class="flex-1">
                            <h3 class="select-none text-xl font-bold text-white mb-2 group-hover:text-green-300 transition-colors">
                                Músicas Curtidas
                            </h3>
                            <p class="select-none text-gray-400 text-sm group-hover:text-gray-300 transition-colors">
                ...
                <div wire:click="toggleDelete"
                    ...
                    <div class="flex items-center gap-4">
                        <div class="relative">
                            ...
                            <div
                                class="floating-icon w-16 h-16 bg-gradient-to-br from-orange-500 to-red-500 rounded-full flex items-center justify-center shadow-lg">
                                <svg class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div class="px-8">
        ...
        <div @class([
                    'overflow-y-auto cst-scrollbar ',
                    'max-h-[550px]' => !$canEditPlaylist,
                    'max-h-[450px]' => $canEditPlaylist
        ])                      x-data="{ isLoading: false }"
             x-on:scroll="if (!isLoading && $el.scrollTop + $el.clientHeight >= $el.scrollHeight - 10 && {{ count($this->playlistTracks['tracks']) }} < {{ $this->playlistTracks['total'] }}) {
           isLoading = true;
           $wire.loadMore().then(() => isLoading = false);
       }">
            <div class="space-y-1">
                @foreach($this->playlistTracks['tracks'] as $key => $track)
                    <label for="editmusics{{$key}}" @class([
